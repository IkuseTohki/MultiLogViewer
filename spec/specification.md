...
- **動作仕様**:
  ...
  - 未指定（null または空）の場合は、**期間によるフィルタリングを行わず、すべてのログ**を対象とします。
- **サポートする書式**:
...
- **安全なデータアクセス**: UI バインディングや変換処理において、`AdditionalData`内のキーが存在しない場合に例外が発生するのを防ぐため、`LogEntry`自身がインデクサ (`entry[key]`) を提供します。キーが存在しない場合は空文字を返します。
...
### 2.15 トレースログ機能 (トラブルシューティング)

- **概要**: アプリケーションの動作詳細（ログの読み込み、コピー処理の実行状況など）をファイルに記録する機能。
- **動作仕様**:
  - `AppSettings.yaml` の `enable_trace_log` が `true` の場合にのみ有効。
  - 実行バイナリと同じディレクトリに `trace.log` を生成し、日時とともに詳細な動作情報を出力する。
  - 通常の利用ではパフォーマンス維持のため無効（false）を推奨。
