...
- **操作**:
  ...
  - **ナビゲーション**:
    - `F2` キー: 次の（現在位置より後の）ブックマークされた行へジャンプする。
    - `Shift+F2` キー: 前の（現在位置より前の）ブックマークされた行へジャンプする。
    - 末尾（または先頭）に達した場合は、ループして最初（または最後）のブックマークへ移動する。
  - **一括操作**: すべてのブックマークを一度に解除する機能を提供する。
  - **ブックマークフィルタ**: ブックマークされた行のみを表示するフィルタ機能。
- **表示**:
  - **行ヘッダー**: ブックマークされた行の左端（行ヘッダー領域）に、しおりまたは星のアイコンを表示する。
  - **ブックマーク一覧パネル**: サイドパネル（詳細パネルと切り替え、または併設）に、現在ブックマークされている行の一覧を表示する。一覧をクリックすることで該当行へジャンプできる。
- **付加情報**:
  - **メモ機能**: 各ブックマークに対して短いメモ（調査状況や重要度の備忘録など）を付与できる。メモの内容は一覧パネルで確認・編集可能。
- **解析支援**:
  - **ダイジェスト・ビュー**: ブックマークされた行のみを抽出して時系列に並べ、特別な統計情報を付与して表示する専用画面。
  - **経過時間（時間差）表示**: 1 つ前のブックマーク行からの経過時間（Δt）を自動計算して表示する。これにより、「エラーの 5 分前にこの予兆が出ていた」といった時間的な相関分析を容易にする。
  - **相対時間表示**: 最初のブックマークを 0 秒とした、累積の経過時間も表示可能。
- **管理**:
