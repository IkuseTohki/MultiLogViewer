...
## トレースログ機能 (トラブルシューティング)

アプリケーションが期待通りに動作しない場合や、コピー機能などの詳細な挙動を調査したい場合に、詳細な動作ログをファイルに出力できます。

`AppSettings.yaml` で以下のように設定してください。

```yaml
# トラブルシューティング時のみ true に設定
enable_trace_log: true
```

設定を `true` にしてアプリを起動すると、実行フォルダに `trace.log` が生成されます。調査が終わったら、パフォーマンスのために `false` に戻すことをオススメします。

# 1 ヶ月前の同日 00:00:00 以降

log_retention_limit: "-1m"
